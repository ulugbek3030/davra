...
export default function PlansPage() {
  return (
    <div>
      <h1 className="font-display text-2xl font-bold sm:text-3xl">Тарифы и продвижение</h1>
      <p className="mt-1 text-muted">Больше возможностей и клиентов для вашей чайханы.</p>
          ...
          >
            ...
            <div className="mt-4 flex items-end gap-1">
              <span className="font-display text-3xl font-bold">
                {p.price === 0 ? "Бесплатно" : formatSom(p.price)}
              </span>
              {p.price > 0 && <span className="pb-1 text-sm text-muted">/ мес</span>}
            </div>
            ...
            >
              {p.current ? "Текущий тариф" : `Перейти на ${p.name}`}
            </button>
          </div>
        ))}
      </div>
      ...
      {/* Promotion */}
      <div className="mt-10">
        <h2 className="font-display text-xl font-bold">Продвижение</h2>
        <p className="mt-1 text-muted">Поднимите заведение выше в поиске и привлеките больше броней.</p>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-3">
          {BOOST_PACKAGES.map((b) => (
            <div key={b.id} className="rounded-3xl border border-sand bg-surface p-5">
              ...
              <div className="text-sm text-muted">{b.period}</div>
              <div className="mt-3 flex items-center justify-between">
                <span className="font-display text-lg font-bold">{formatSom(b.price)}</span>
